...
import 'package:hungerz/cubits/cart_cubit/cart_state.dart'; // Adaptez chemin
...
import 'package:hungerz/models/place_model.dart'; // Adaptez chemin/nom
...
class _ViewCartState extends State<ViewCart> {
  ...
  Widget build(BuildContext context) {
      ...
      body: BlocListener<OrderCubit, OrderState>(
       ...
       if (orderState is OrderPlacementSuccessNavigateToOrders) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
             SnackBar(content: Text("Commande passée avec succès !"), backgroundColor: Colors.green),
          );
          ...
          Future.delayed(Duration(milliseconds: 500), () {
             if (mounted) { // Vérifier si toujours monté
          ...
          ScaffoldMessenger.of(context).showSnackBar(
             SnackBar(content: Text("Erreur commande: ${orderState.error}"), backgroundColor: Colors.red),
          );
       }
    },
                      ...
                      child: Column(
                        ...
                        children: <Widget>[
                          Container(
                              ...
                              color: Theme.of(context).scaffoldBackgroundColor, // Fond pour cacher la liste
                              padding: EdgeInsets.only(left: 20.0, right: 12.0, top: 13.0, bottom: 13.0),
                              child: BlocBuilder<ProfileCubit, ProfileState>( // <-- Écoute ProfileCubit
                                 builder: (context, profileState) {
                                    AddressModel? activeAddress;
                                    if (profileState is ProfileLoaded) {
                                       activeAddress = profileState.activeDisplayAddress;
                                    }
                                    ...
                                    if (activeAddress != null) {
                                       return _buildAddressDisplay(
                                          ...
                                          icon: Icons.location_on_outlined, // Ou une icône basée sur le type?
                                          locale: locale,
                                        );
                                    } else {
                                       ...
                                       return BlocBuilder<LocationCubit, LocationState>(
                                         builder: (context, locationState) {
                                            String addressText = locale.setLocation ?? "Définir l'adresse de livraison";
                                            IconData addressIcon = Icons.gps_not_fixed;
                                            if (locationState is LocationLoading) { addressText = "Localisation en cours..."; }
                                            else if (locationState is LocationLoaded) {
                                               addressText = locationState.simpleAddress;
                                               addressIcon = Icons.my_location; // Icône différente pour GPS
                                            }
                                            else if (locationState is LocationPermissionDenied) { addressText = "Permission localisation refusée"; }
                                            else if (locationState is LocationServiceDisabled) { addressText = "Service localisation désactivé"; }
                                            else if (locationState is LocationError) { addressText = "Erreur localisation"; }
                          ...
                          BottomBar(
                            text:
                                "Payer ${finalAmount.toStringAsFixed(2)} DA", // Total final local
                            onTap: () {
    ...
    if (state.items.isNotEmpty) {
      ...
      AddressModel? activeAddress; // Peut être null pour Take Away
      if (profileState is ProfileLoaded) {
         activeAddress = profileState.activeDisplayAddress;
      }

      // 3. Vérification : Pour la livraison, une adresse est requise
      if (_selectedDeliveryMethod == DeliveryMethod.delivery && activeAddress == null) {
          ScaffoldMessenger.of(context).showSnackBar(
             SnackBar(content: Text( "Veuillez sélectionner une adresse de livraison."), backgroundColor: Colors.orange),
          );
          return; // Arrêter si l'adresse manque pour la livraison
      }

      // 4. Méthode de livraison et paiement depuis l'état local
      final DeliveryMethod deliveryMethod = _selectedDeliveryMethod;
      final PaymentMethod paymentMethod = _selectedPaymentMethod;
      ...
      print(" - Adresse: ${activeAddress?.address ?? 'N/A'}");
      print(" - Nb Items: ${currentItems.length}");

      // --- Appel à OrderCubit ---
      context.read<OrderCubit>().placeOrder(
         items: currentItems,
         deliveryAddress: activeAddress, // Null si Take Away
      ...
      // Pour l'instant, on ne navigue plus directement ici.
       ...
       ScaffoldMessenger.of(context).showSnackBar( SnackBar(content: Text("Votre panier est vide.")),);
    }
                            },
                          ),
                        ],
  ...
  Widget _buildDeliveryOptionCard({
   ...
   Widget _buildAddressDisplay({
      ...
      required AppLocalizations locale,
       ...
       return InkWell( // Rendre l'adresse cliquable pour la changer
          onTap: () async {
              ...
              if (result != null && context.mounted) {
                  ...
                  if (result.latitude != null && result.longitude != null) {
                      ...
                      context.read<ProfileCubit>().setActiveDisplayAddress(selectedAddrModel);
          ...
          child: Row(
             children: <Widget>[
                Icon(icon, color: Color(0xffc4c8c1), size: 16), // Icône un peu plus grande
                SizedBox(width: 11.0),
                Expanded( // Pour que le texte utilise ellipsis si besoin
                  child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                         Text(label, // Le label (ex: "Maison", "Livraison à")
                             style: Theme.of(context).textTheme.bodySmall!.copyWith(color: kDisabledColor, fontWeight: FontWeight.bold)),
                         SizedBox(height: 2),
                         Text(address, // L'adresse formatée
                             style: Theme.of(context).textTheme.bodySmall!.copyWith(fontSize: 11.7, color: Color(0xffb7b7b7)),
                             maxLines: 1,
                ...
                Icon(Icons.arrow_drop_down, color: Colors.grey), // Indicateur pour changer
             ],
          ),
       );
   }
