...
class _DishDetailPageState extends State<DishDetailPage> {
  ...
  void _addToCart() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('${widget.dish.name} (x$_quantity) ajouté au panier!'),
                      ...
                      BlocSelector<ProfileCubit, ProfileState, bool>(
                        selector: (state) {
                          ...
                          if (state is ProfileLoaded) {
                            ...
                            return state.user.favorites?.contains(widget.dish.id) ?? false;
                          }
                          return false; // Non favori par défaut ou si état non chargé
                        },
                        builder: (context, isFavorite) {
                          print("DishDetailPage: Rebuild bouton favori pour ${widget.dish.name}. Est favori: $isFavorite");
                          return IconButton(
                            icon: Icon(
                            ...
                            onPressed: () {
                              ...
                              // Assurez-vous que votre modèle 'dish' a un champ 'id' (String)
                              context.read<ProfileCubit>().toggleFavorite(widget.dish.id!);
                            },
                          );
                        },
                      ),
