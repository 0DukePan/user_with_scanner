...
import 'package:hungerz/cubits/cart_cubit/cart_state.dart';
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: InkWell(
  onTap: () async { // Rendre async pour attendre le retour
    print("HomePage AppBar: Navigation vers AddressSearchPage");
    final result = await Navigator.push<Place?>( // Attendre un résultat Place ou null
      context,
    ...
    if (result != null && context.mounted) {
      ...
      if (result.latitude != null && result.longitude != null) {
          AddressModel selectedAddrModel = AddressModel(
             ...
             label: result.name, // Utiliser le nom du lieu comme label par défaut
          ...
          print("HomePage: Coordonnées manquantes dans le résultat Place, ne peut pas définir comme active.");
          // Afficher une erreur ?
      }
        ...
        // Pour l'instant, on ne fait rien, l'adresse active précédente reste.
         print("HomePage: Retour depuis AddressSearchPage sans sélection.");
        ...
        child: Column(
          ...
          children: [
             ...
             BlocBuilder<ProfileCubit, ProfileState>(
                 buildWhen: (prev, curr) => prev.runtimeType != curr.runtimeType || (prev is ProfileLoaded && curr is ProfileLoaded && prev.activeDisplayAddress != curr.activeDisplayAddress),
                 builder: (context, profileState){
                     String topLabel =  "Adresse Actuelle";
                     if (profileState is ProfileLoaded && profileState.activeDisplayAddress != null) {
                        // Si une adresse active existe, utiliser son label comme titre principal
                         topLabel = profileState.activeDisplayAddress!.label;
                     }
                     return Text( topLabel, style: Theme.of(context).textTheme.bodySmall?.copyWith(fontSize: 11, color: Colors.grey[500]),);
                 }
             ),
            SizedBox(height: 1),
               ...
               builder: (context, profileState) {
                 ...
                 if (profileState is ProfileLoaded && profileState.activeDisplayAddress != null) {
                   return Row(
                     mainAxisSize: MainAxisSize.min,
                     children: [
                       Flexible(child: Text(profileState.activeDisplayAddress!.address, style: Theme.of(context).textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.bold), overflow: TextOverflow.ellipsis, maxLines: 1)),
                   ...
                   return BlocBuilder<LocationCubit, LocationState>(
                     builder: (context, locationState) {
                       ...
                       if(locationState is LocationLoaded) displayText = locationState.simpleAddress;
                       // ... autres états ...
                       return Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                             Flexible(child: Text(displayText, style: Theme.of(context).textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.bold, /*...*/), overflow: TextOverflow.ellipsis, maxLines: 1)),
                             Icon(Icons.arrow_drop_down, size: 20, color: Colors.grey[700]),
                          ],
                       );
                     },
      ...
      body: BlocBuilder<DishesCubit, DishesState>(builder: (context, state) {
        if (state is DishesInitial || state is DishesLoading) {
          ...
          if (profileState is ProfileLoaded) {
            recommendedIds = profileState.user.recommandations; // Peut être null ?
          }
          if (recommendedIds != null && recommendedIds.isNotEmpty) {
            ...
            recommendedDishes = allDishes
                ...
                .toList();
            popularDishes = allDishes.where((dish) => dish.isPopular!).toList();
          }
